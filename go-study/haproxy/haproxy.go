...
func (g *haproxy) gatherServerSocket(addr string) error {
	...
	return importCsvResult(c, socketPath)
}

func (g *haproxy) gatherServer(addr string) error {
	if !strings.HasPrefix(addr, "http") {
		return g.gatherServerSocket(addr)
	}
	...
	return importCsvResult(res.Body, u.Host)
}

func importCsvResult(r io.Reader, host string) error {
	csv := csv.NewReader(r)
...
func main() {
	...
	log.Println("Get status from http restful interface")
	gather.gatherServer("http://admin:admin@localhost:5091/")

	log.Println("Get status from socket commadn")
	gather.gatherServerSocket("/data/run/haproxy/admin.sock")
}
